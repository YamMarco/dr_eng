<script lang="ts">
	...
	// Highlighting: a student selects part of a line and picks a color from a
	// small floating palette; segments are computed from plain-text offsets,
	// so highlighting only lines up correctly on passage text with no inline
	// markdown (the norm for reading passages). State is per-mount only - a
	// closed overlay/reopened screen starts fresh, which is fine for a
	// scratch annotation tool.
	type MarkColor = 'yellow' | 'green' | 'pink' | 'blue';
	type Highlight = { lineNumber: number; start: number; end: number; color: MarkColor };
	let highlights = $state<Highlight[]>([]);
	let pendingSelection = $state<{ lineNumber: number; start: number; end: number } | null>(null);
	...
	function segmentsFor(line: Line): { text: string; color: MarkColor | null }[] {
		const spans = highlights
			.filter((h) => h.lineNumber === line.lineNumber)
	...
	function onMouseUp() {
		...
		const startP = startEl?.closest('p[data-line]');
		const endP = endEl?.closest('p[data-line]');
		if (!startP || !endP || startP !== endP) return;

		const lineNumber = Number(startP.getAttribute('data-line'));
		...
		pendingSelection = { lineNumber, start, end };
		toolbarPos = { top: rect.top - 44, left: rect.left + rect.width / 2 };
	}

	function applyColor(color: MarkColor | null) {
		if (!pendingSelection) return;
		const { lineNumber, start, end } = pendingSelection;
		const otherLines = highlights.filter((h) => h.lineNumber !== lineNumber);
		const kept: Highlight[] = [];
		for (const h of highlights.filter((h) => h.lineNumber === lineNumber)) {
		...
		highlights = color
			? [...otherLines, ...kept, { lineNumber, start, end, color }]
			: [...otherLines, ...kept];
...
<div class="flex flex-col" dir="ltr" role="presentation" onmouseup={onMouseUp}>
	{#each lines as line (line.lineNumber)}
		<div class="flex items-start gap-2 {line.roman ? 'mt-3' : ''}">
			...
			<p data-line={line.lineNumber} class="flex-1 leading-relaxed">
				{#each segmentsFor(line) as seg, i (i)}
					{#if seg.color}
						<span class="{MARK_BG[seg.color]} text-mark-ink rounded-xs"
							><Md text={seg.text} /></span
						>
					{:else}
						<Md text={seg.text} />
					{/if}
	...
	>
		{#each MARK_NAMES as name (name)}
			<button
				...
				class="{MARK_BG[name]} h-5 w-5 shrink-0 rounded-full ring-1 ring-line/50 transition active:scale-90"
			></button>
		{/each}
		<button
			type="button"
